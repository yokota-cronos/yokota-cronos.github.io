...
{% for p in pubs %}
  ...
  <div class="pub-entry">
    <div class="pub-main">
      ...
      {% assign venue = p.publisher | default: p.venue %}{% if venue and venue != "" %}<div class="pub-venue">{{ venue }}{% if p.volume and p.volume != "" %}, {{ p.volume }}{% if p.number and p.number != "" %}({{ p.number }}){% endif %}{% endif %}{% if p.pages and p.pages != "" %}, pp.{{ p.pages }}{% endif %}{% if p.year and p.year != "" %} ({{ p.year }}){% endif %}</div>{% endif %}
      <div class="pub-links">
