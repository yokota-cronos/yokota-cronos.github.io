...
{% for m in site.data.members %}
  <div class="member-card{% if m.bio and m.bio != '' %} has-bio{% endif %}" tabindex="0">
    ...
    <div class="member-links">
      ...
      {% if m.utokyo and m.utokyo != "" %}<a class="ico ico-utokyo" href="{{ m.utokyo }}" title="東京大学" target="_blank" rel="noopener"><i class="fas fa-university" aria-hidden="true"></i><span class="sr-only">東京大学</span></a>{% endif %}
...
{% for p in grp.items %}
  ...
  <div class="pub-entry">
    ...
    {% unless pgif and pgif != "" %}{% assign pleft = site.data.pub_left_images[p.title] %}{% if pleft and pleft != "" %}<div class="pub-gif pub-gif--static">{% assign lpdf = p.pdf | default: p.paper %}{% if lpdf and lpdf != "" %}<a href="{% if lpdf contains 'http' %}{{ lpdf }}{% else %}{{ base_path }}/paper/{{ lpdf }}{% endif %}" target="_blank" rel="noopener" title="PDF を開く"><img loading="lazy" src="{{ base_path }}/images/publications/{{ pleft }}" alt=""></a>{% else %}<img loading="lazy" src="{{ base_path }}/images/publications/{{ pleft }}" alt="">{% endif %}</div>{% endif %}{% endunless %}
    <div class="pub-main">
      ...
      {% assign venue = p.publisher | default: p.venue %}{% assign vdoi = p.doi | default: p.url %}{% assign haslink = false %}{% if vdoi and vdoi != "" %}{% assign haslink = true %}{% endif %}{% if p.arxiv and p.arxiv != "" %}{% assign haslink = true %}{% endif %}{% if venue and venue != "" and haslink %}<div class="pub-venue"><span class="pub-venue-name">{{ venue }}</span>{% if p.volume and p.volume != "" %}, {{ p.volume }}{% if p.number and p.number != "" %}({{ p.number }}){% endif %}{% elsif p.number and p.number != "" %}, Article No. {{ p.number }}{% endif %}{% if p.pages and p.pages != "" %}, pp.{{ p.pages }}{% endif %}{% if p.year and p.year != "" %} ({{ p.year }}){% endif %}</div>{% endif %}
      <div class="pub-links">
